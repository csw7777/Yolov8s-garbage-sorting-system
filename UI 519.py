...
"")
        ...
        self.zxd_score.setLocale(QtCore.QLocale(QtCore.QLocale.English, QtCore.QLocale.UnitedStates))
        self.zxd_score.setText("")
...
"}")
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.zxd_score = QtWidgets.QLabel("置信度: 0.25", self)
#         self.zxd_score.setGeometry(QtCore.QRect(320, 500, 100, 20))
        # 获取水平滑动条的x坐标和宽度，以便我们可以将标签放在滑动条的右侧
        slider_x = self.horizontalSlider.x()
        slider_width = self.horizontalSlider.width()

        # 计算标签的x坐标，使其与滑动条的右侧对齐
        label_x = slider_x + slider_width + 5  # 加5作为间距

        # 设置置信度标签的位置和大小，紧随滑动条右侧
        self.zxd_score.setGeometry(QtCore.QRect(label_x, 500, 100, 20))
        self.zxd_score.setAlignment(QtCore.Qt.AlignCenter)

        # 绑定滑动条的值改变事件
        self.horizontalSlider.valueChanged.connect(self.update_confidence_threshold)

        self.conf_threshold = 0.25  # 初始化置信度阈值
